...
def build_from_config(cfg: Dict[str, Any]) -> TceAdapterArgs:
    ...
    if found_legacy_runtime:
        raise ValueError(
            ...
            "Use enable_rq3_apply_service_qa / rq3_apply_fail_on_missing_pack / rq3_apply_save_prompt_and_raw.".format(
                ", ".join(found_legacy_runtime)
            )
        )
    legacy_bp_keys = {"rq3_pair_count_per_key", "rq3_retrieval_top_k"}
    found_legacy_bp = sorted(k for k in legacy_bp_keys if k in baseline_params)
    if found_legacy_bp:
        raise ValueError(
            "Legacy baseline_params RQ3 keys are no longer supported: {}. "
            "Use rq3_apply_items_per_key / rq3_apply_retrieval_top_k.".format(
    ...
    return TceAdapterArgs(
        ...
        rq3_apply_fail_on_missing_pack=coerce_bool(_get(runtime, "rq3_apply_fail_on_missing_pack", False)),
...
def resolved_payload(adapter_args: TceAdapterArgs, config_path: Path = None) -> Dict[str, Any]:
    payload = {
        "runtime": {
            ...
            "rq3_apply_fail_on_missing_pack": adapter_args.rq3_apply_fail_on_missing_pack,
