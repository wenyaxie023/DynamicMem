...
| `.env` | Contains Azure credentials (`OPENAI_API_KEY`, `OPENAI_BASE_URL`). |
...
- **Graph Data**: `outputs/{user_id}_large/gpt-5-mini_text-embedding-3-small/graph.pickle`
- **OpenIE Cache**: `outputs/{user_id}_large/openie_results_ner_gpt-5-mini.json`
...
- **Resumption**: If a process stops, simply re-running the script will automatically resume from the last checkpoint in `indexing_checkpoint.json`.
