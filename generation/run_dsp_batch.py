...
from generation.dsp_config import (
    build_from_config,
    deep_merge,
    load_yaml,
    normalize_users,
...
def _load_benchmark_builder_api():
    module_name = "data_construction.build_dynamic_state_prediction_benchmark"
    try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch DSP runner with user templating")
    parser.add_argument("--config", type=Path, required=True, help="Batch YAML config")
    parser.add_argument("--defaults", type=Path, default=Path("configs/dsp.default.yaml"), help="YAML defaults")
