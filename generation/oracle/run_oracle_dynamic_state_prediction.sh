...
# Batch runner for Oracle dynamic state prediction generation.
# Produces: generation/oracle/results/<user_id>/prediction/dynamic_state_prediction_results.json
...
OUTPUT_ROOT="$PROJECT_ROOT/generation/oracle/results"
...
for user in "${USERS[@]}"; do
  ...
  if [[ ! -f "$benchmark_path" ]]; then
    echo "[ORACLE-DSP] SKIP $user_dir: benchmark not found"
    continue
  fi

  if [[ ! -f "$app_logs_path" ]]; then
    echo "[ORACLE-DSP] SKIP $user_dir: app_log_large not found: $app_logs_path"
    continue
  fi

  echo "[ORACLE-DSP] user=$user_dir model=$LLM_MODEL max_visible_logs=${MAX_VISIBLE_LOGS:-full}"

  cmd=(
    "$PYTHON_BIN" -m generation.oracle.dynamic_state_prediction
