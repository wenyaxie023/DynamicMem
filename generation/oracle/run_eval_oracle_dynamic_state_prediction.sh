...
PREDICTION_ROOT="$PROJECT_ROOT/generation/oracle/results"
...
for user in "${USERS[@]}"; do
  ...
  if [[ ! -f "$benchmark_path" ]]; then
    echo "[ORACLE-DSP-EVAL] SKIP $user_dir: benchmark not found: $benchmark_path"
    continue
  fi

  if [[ ! -f "$prediction_path" ]]; then
    echo "[ORACLE-DSP-EVAL] SKIP $user_dir: prediction not found: $prediction_path"
    continue
  fi

  echo "[ORACLE-DSP-EVAL] user=$user_dir llm_judge=$ENABLE_LLM_JUDGE"
