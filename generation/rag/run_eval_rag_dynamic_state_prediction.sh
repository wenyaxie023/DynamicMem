...
for user in "${USERS[@]}"; do
  ...
  if [[ ! -f "$benchmark_path" ]]; then
    echo "[RAG-DSP-EVAL] SKIP $user_dir: benchmark not found: $benchmark_path"
    continue
  fi

  if [[ ! -f "$prediction_path" ]]; then
    echo "[RAG-DSP-EVAL] SKIP $user_dir: prediction not found: $prediction_path"
    continue
  fi

  echo "[RAG-DSP-EVAL] user=$user_dir llm_judge=$ENABLE_LLM_JUDGE"
