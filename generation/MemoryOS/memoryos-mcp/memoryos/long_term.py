...
try:
    from .utils import (
        get_timestamp, get_embedding, normalize_vector, ensure_directory_exists,
...
class LongTermMemory:
    ...
    def _search_knowledge_deque(self, query, knowledge_deque: deque, threshold=0.1, top_k=5):
        ...
        if self.embedding_backend == "bm25":
            entries = [entry for entry in knowledge_deque if entry.get("knowledge")]
            if not entries:
                return []
            texts = [entry["knowledge"] for entry in entries]
