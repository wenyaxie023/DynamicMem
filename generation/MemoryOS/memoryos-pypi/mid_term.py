...
try:
    from .utils import (
        ...
        resolve_embedding_backend, compute_bm25_scores
    )
except ImportError:
    from utils import (
        get_timestamp, generate_id, get_embedding, normalize_vector,
...
class MidTermMemory:
    def __init__(self, file_path: str, client: OpenAIClient, max_capacity=2000, embedding_model_name: str = "all-MiniLM-L6-v2", embedding_model_kwargs: dict = None):
        ...
        self.embedding_backend = resolve_embedding_backend(self.embedding_model_name, self.embedding_model_kwargs)
        self.load()
    ...
    def add_session(self, summary, details, summary_keywords=None):
        ...
        use_bm25 = self.embedding_backend == "bm25"
        summary_vec = []
        if not use_bm25:
    ...
    def insert_pages_into_session(self, summary_for_new_pages, keywords_for_new_pages, pages_to_insert, 
        ...
        if use_bm25:
            ...
            summaries = [self.sessions[sid]["summary"] for sid in session_ids]
            scores = compute_bm25_scores(summaries, summary_for_new_pages)
            if scores.size:
                max_score = float(scores.max()) if scores.size else 0.0
                if max_score > 0:
                    best_idx = int(scores.argmax())
                    best_sid = session_ids[best_idx]
                    best_overall_score = float(scores[best_idx]) / max_score
        else:
    ...
    def search_sessions(self, query_text, segment_similarity_threshold=0.1, page_similarity_threshold=0.1,
        ...
        if use_bm25:
            summaries = [self.sessions[sid]["summary"] for sid in session_ids]
            scores = compute_bm25_scores(summaries, query_text)
            if scores.size == 0:
                return []
            ...
            top_indices = np.argsort(scores)[-min(top_k_sessions, len(session_ids)):]
            for idx in top_indices[::-1]:
                session_id = session_ids[idx]
                session = self.sessions[session_id]
                session_relevance_score = float(scores[idx]) / max_score
                ...
                page_texts = [
                    f"User: {page.get('user_input','')} Assistant: {page.get('agent_response','')}"
                    for page in pages
                ]
                page_scores = compute_bm25_scores(page_texts, query_text) if pages else np.array([], dtype=np.float32)
                page_max = float(page_scores.max()) if page_scores.size else 0.0

                for page_idx, page in enumerate(pages):
                    page_score = float(page_scores[page_idx]) / page_max if page_max > 0 else 0.0
                    if page_score >= page_similarity_threshold:
                        matched_pages_in_session.append({"page_data": page, "score": page_score})
