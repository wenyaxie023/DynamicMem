...
def _maybe_eval_relative_path(expr: str, node: Node) -> Optional[Tuple[bool, Any]]:
...
def eval_template(value: Any, node: Node, domain: str) -> Any:
    if isinstance(value, str):
        if value.startswith("#"):
            ...
            path_eval = _maybe_eval_relative_path(inner, node)
            if path_eval is not None:
                matched, rel_value = path_eval
                if matched:
                    return rel_value
...
def apply_emit(spec: AtomSpec, node: Node, domain: str) -> Atom:
    ...
    path = emit.get("path") if "path" in emit else None
    atom_id = stable_atom_id(spec.rule_id, path)
    extra = {
        k: v
        for k, v in emit.items()
        if k not in {"anchor", "key", "value", "time_window", "domain", "path"}
    }
    return Atom(
        atom_id=atom_id,
        domain=domain_value,
...
def main() -> int:
    ...
    #    atoms.extend(atomize(schema, specs))
    atoms = atomize(schema, specs)
