...
class BaseApp(ABC):
    ...
    def _record_common_state(
        ...
        # Extract top-level IDs (excluding conversation_id which is managed per-conversation in LLMAssistantApp)
        for key in ("session_id", "device_id", "account_id", "order_id"):
...
class AmazonApp(BaseApp):
    ...
    API_MODELS = {
        ...
        "AddToWishlist": (app_models.AddToWishlistInput, app_models.AddToWishlistOutput),
    ...
    def _update_state(self, event, request_payload, response_payload) -> None:
        ...
        if api_name == "SearchProducts":
            ...
            if review and product_id:
                ...
                self._append_to_state_history("reviews", review_record, MAX_HISTORY_MEDIUM)
...
class SpotifyApp(BaseApp):
    ...
    def _update_state(self, event, request_payload, response_payload) -> None:
        ...
        if api_name == "PlaySong":
            ...
            playlist = response_payload.get("playlist", {})
            if playlist:
                playlist_id = playlist.get("playlist_id")
                playlists = self.state.setdefault("playlists", [])
                # Update existing playlist or add new one
                updated = False
                for idx, existing in enumerate(playlists):
                    if existing.get("playlist_id") == playlist_id:
                        ...
                        break
                if not updated:
                    self._append_to_state_history("playlists", playlist, MAX_HISTORY_SMALL)
...
class FitbitApp(BaseApp):
    ...
    def _update_state(self, event, request_payload, response_payload) -> None:
        ...
        if api_name == "LogWorkout":
            ...
            goals = response_payload.get("goals", [])
            if goals:
...
class RobinhoodApp(BaseApp):
    ...
    def _update_state(self, event, request_payload, response_payload) -> None:
        ...
        if api_name in ("BuyStock", "SellStock"):
            ...
            watchlist_items = response_payload.get("watchlist", [])
            if watchlist_items:
                # Extract symbols from watchlist items and update state
                symbols = [item.get("symbol") for item in watchlist_items if item.get("symbol")]
                self.state["watchlist"] = symbols[-MAX_HISTORY_LARGE:]
...
class GmailApp(BaseApp):
    ...
    def _update_state(self, event, request_payload, response_payload) -> None:
        ...
        elif api_name == "GetInbox":
            emails = response_payload.get("emails", [])
            if emails:
                self.state["inbox"] = emails[-MAX_HISTORY_SMALL:]
...
class LinkedInApp(BaseApp):
    ...
    def _update_state(self, event, request_payload, response_payload) -> None:
        ...
        if api_name == "PostUpdate":
        ...
        elif api_name == "LikePost":
            post_id = request_payload.get("post_id", "")
            if post_id:
                liked_posts = self.state.setdefault("liked_posts", [])
                if post_id not in liked_posts:
                    self._append_to_state_history("liked_posts", post_id, MAX_HISTORY_MEDIUM)
        elif api_name == "ApplyJob":
            ...
            if job_id:
                ...
                self._append_to_state_history("job_applications", application, MAX_HISTORY_SMALL)
...
class InstagramApp(BaseApp):
    ...
    def _update_state(self, event, request_payload, response_payload) -> None:
        ...
        elif api_name == "GetFollowing":
            following_list = response_payload.get("following", [])
            if following_list:
                self.state["following"] = following_list[-MAX_HISTORY_LARGE:]
        elif api_name == "LikePost":
            post_id = request_payload.get("post_id", "")
            if post_id:
                liked_posts = self.state.setdefault("liked_posts", [])
...
class LLMAssistantApp(BaseApp):
    ...
    def _update_state(self, event, request_payload, response_payload) -> None:
        ...
        if api_name == "CreateConversation":
            ...
            conversations = self.state.setdefault("conversations", [])
            conv_found = None
            conv_index = -1
            for idx, conv in enumerate(conversations):
                if conv.get("conversation_id") == conv_id:
                    ...
                    conv_index = idx
                    break
            if conv_found is None:
                # Conversation was truncated from history, re-create it with current messages
                initial_messages = []
                if user_msg:
                    initial_messages.append(user_msg)
                if assistant_msg:
                    initial_messages.append(assistant_msg)
                conv_found = {
                    "conversation_id": conv_id,
                    "messages": initial_messages,
                    "created_at": timestamp,
                }
                conversations.append(conv_found)
            else:
                # Move existing conversation to end (most recently active)
                conversations.pop(conv_index)
                conversations.append(conv_found)
                # Add new messages
                messages = conv_found.setdefault("messages", [])
                if user_msg:
                    messages.append(user_msg)
                if assistant_msg:
                    messages.append(assistant_msg)
                if len(messages) > MAX_HISTORY_SMALL * 2:
                    conv_found["messages"] = messages[-(MAX_HISTORY_SMALL * 2):]
            # Keep only the most recently active N conversations
            if len(conversations) > MAX_HISTORY_SMALL:
                self.state["conversations"] = conversations[-MAX_HISTORY_SMALL:]
