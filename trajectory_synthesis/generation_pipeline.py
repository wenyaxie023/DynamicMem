...
from trajectory_synthesis.events_chain_generator import (
    EventsChainRequest,
    generate_events_chain,
)
from trajectory_synthesis.app_system import (
    APP_API_SCHEMAS,
    AppRegistry,
...
from trajectory_synthesis.prompt_templates import (
    ...
    Life_Context_Baseline_Prompt,
    Life_Context_Delta_Prompt,
    TIME_CONFLICT_RESOLUTION_PROMPT,
    app_log_prompt,
    in_domain_data_review_revise_prompt,
...
def _expand_habit_schedule_dates(habit: object, time_range: object) -> object:
    if not isinstance(habit, dict):
        return habit
    start_date, end_date = _parse_window_date_range(time_range)
    if not start_date or not end_date:
        return habit
    schedule = habit.get("schedule") or {}
    if not isinstance(schedule, dict) or not schedule.get("frequency_type"):
        return habit
    occurrences = _dates_from_schedule(schedule, start_date, end_date)
    if not occurrences:
        return habit
    expanded = deepcopy(habit)
    expanded["schedule_dates"] = [dt.isoformat() for dt in occurrences]
    return expanded
...
class GenerationPipeline:
    ...
    def generate_events_chain_for_domain(
        ...
        *,
        ...
        # user_life_contexts carries "baseline" and "by_window".
        ...
        # --------- Resolve window states + context maps ---------
        slug = _slugify(domain.domain_name)
        ...
        # Build per-window background + summary maps.
        ...
        # --------- Stable-state sampling configuration ---------
        rng = random.Random()
        stable_state_convert_probability = 0.5
        stale_sample_probability = max(
            0.0, min(1.0, stable_state_convert_probability)
        )
        ...
        # ---------- Helpers (kept together for readability) ----------
        ...
        def _init_state_tracker() -> Dict[str, Dict[str, Dict[str, Dict[str, object]]]]:
            ...
            user_attrs_section: Dict[str, object] = {}
            user_attrs_state = initial_state.get("user_attributes_state") or {}
            if isinstance(user_attrs_state, dict):
                singular = user_attrs_state.get("singular")
                collections = user_attrs_state.get("collections")
                if isinstance(singular, dict):
                    user_attrs_section.update(singular)
                if isinstance(collections, dict):
                    user_attrs_section.update(collections)
                if not user_attrs_section:
                    legacy_entries = _extract_initial_state_entries(
                        user_attrs_state.get("initial")
                    )
                    for name, entry in legacy_entries.items():
                        user_attrs_section[name] = entry.get("current_value")
            initial_sections = {
                "user_attributes_state": user_attrs_section,
        ...
        def _materialize_state_table_from_tracker() -> Dict[str, List[Dict[str, object]]]:
            """Convert tracker snapshot into a state_table list structure."""
        ...
        def _build_window_prompt_context(
            *,
            ...
            # Build per-window prompt context: previous summaries + life context + description.
            if idx == 0:
            ...
            *,
            ...
            for state_type in ["user_attributes_state", "habits_state", "preferences_state"]:
                ...
                for item in window_items:
                    ...
                    # Collection-level change entry for list updates (e.g., remove/replace/set).
                    if (
        ...
        def _assign_event_ids_for_window(
            ...
            if event_chains is None:
                event_chains = window_payload.get("events_chain")
            if not isinstance(event_chains, list):
                return
        ...
        for idx, window_state in enumerate(resolved_windows):
            ...
            # Pull window summaries + per-window life context for the prompt.
            ...
            (
                ...
                life_context_prompt_str,
                user_this_window_description,
            ) = _build_window_prompt_context(
                idx=idx, window_id=window_id, window_state=window_state
            )
            ...
            for state_type in ["user_attributes_state", "habits_state", "preferences_state"]:
                ...
                for entry in entries:
                    ...
                    if state_type == "preferences_state":
                        # Hide preference signals from previous_value in prompt payload.
                        prev_value = entry.get("previous_value")
                        if isinstance(prev_value, dict):
                            prev_value = deepcopy(prev_value)
                            prev_value.pop("signals", None)
                            entry["previous_value"] = prev_value
                        elif isinstance(prev_value, list):
                            scrubbed_prev: List[object] = []
            ...
            events_chains_result = generate_events_chain(
                ...
                EventsChainRequest(
                    ...
                    domain_window_state=domain_window_state_payload_for_prompt,
                    world_background=window_world_background
                ),
            )
            events_chain_payload = events_chains_result.data
            if isinstance(events_chain_payload, dict):
                events_chain_payload.setdefault("window_id", window_id)
            ...
            _write_text(prompt_path, events_chains_result.prompt)

            semantic_path = self.output_dir / f"{slug}_events_chain_{window_id}.json"
            _write_json(semantic_path, events_chain_payload)
            legacy_semantic_path = self.output_dir / f"{slug}_events_chain_{window_id}.json"
            _write_json(legacy_semantic_path, events_chain_payload)
...
def debug_generate_events_chain() -> None:
    ...
    with open(base_dir / "context_world_background_2024.txt", "r") as f:
        world_background = f.read()
    with open(output_dir / "dynamic_profiles_conflict_resolved_initial_iter33.json", "r") as f:
        dynamic_profiles = json.load(f)
    with open(output_dir / "life_context_by_window.json", "r") as f:
        user_life_contexts = json.load(f)
    user_full_state_summaries = _build_user_full_state_summaries(dynamic_profiles)
...
def debug_generate_real_data(cutoff_date: Optional[str] = None) -> None:
    ...
    class AppCallPayload(BaseModel):
        """Generic payload for API calls when specific models aren't available."""
    ...
    def _validate_with_api_models(
        app_name: str, 
        api_name: str, 
        input_data: Dict[str, Any], 
        output_data: Dict[str, Any]
    ) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """Validate input/output using specific API Pydantic models if available."""
        ...
        if input_model:
            try:
                validated_input = _pydantic_dump(_pydantic_validate(input_model, input_data))
            except ValidationError as e:
                print(f"  Warning: Input validation failed for {app_name}.{api_name}: {e}")
                validated_input = input_data
        
        if output_model:
            try:
                validated_output = _pydantic_dump(_pydantic_validate(output_model, output_data))
            except ValidationError as e:
                print(f"  Warning: Output validation failed for {app_name}.{api_name}: {e}")
                validated_output = output_data
    ...
    def _expand_event_instances(event: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        If the event has `app_api_variations`, randomly sample one app/api pair
        for each schedule_date instance to simulate realistic habit variation.
        """
        import random
        ...
        if schedule_dates:
            ...
            for date_value in schedule_dates:
                ...
                # If app_api_variations exists, sample one for this instance
                if app_api_variations:
                    sampled_variation = random.choice(app_api_variations)
                    payload["app_name"] = sampled_variation.get("app_name")
                    payload["api_name"] = sampled_variation.get("api_name")
                    # Store the original variations for context
    ...
    def _collect_events(
        ...
        for window_data in windows:
            ...
            for chain_idx, chain in enumerate(window_data.get("event_chains", [])):
                if not isinstance(chain, dict):
                    continue
                if selected_chain_keys is not None:
    ...
    for events_file in events_files:
        ...
        for entry in selected_chains:
            print(
                f"  - {entry['domain_name']} {entry['window_id']} {entry['chain_id']}"
            )
        selected_keys = {(entry["window_id"], entry["chain_idx"]) for entry in selected_chains}
        all_events.extend(_collect_events(domain_data, fallback_domain, selected_keys))
    ...
    app_call_counts: Dict[str, int] = {}
    api_call_counts: Dict[str, int] = {}
    for event in all_events:
        app_name = event.get("app_name")
        api_name = event.get("api_name")
        if not app_name and not api_name:
            continue
        app_label = app_name or "Unknown"
        api_label = api_name or "Unknown"
        app_call_counts[app_label] = app_call_counts.get(app_label, 0) + 1
        api_key = f"{app_label}.{api_label}"
        api_call_counts[api_key] = api_call_counts.get(api_key, 0) + 1

    if app_call_counts:
        print("\nApp call counts:")
        for app_name, count in sorted(
            app_call_counts.items(), key=lambda item: (-item[1], item[0])
        ):
            print(f"  - {app_name}: {count}")

    if api_call_counts:
        print("\nAPI call counts:")
        for api_key, count in sorted(
            api_call_counts.items(), key=lambda item: (-item[1], item[0])
        ):
    ...
    for idx, event in enumerate(all_events, 1):
        event["event_id"] = f"e{idx:04d}"
    
    # Track generated app logs by chain for consistency
    chain_logs_history: Dict[str, List[Dict[str, Any]]] = {}
    
    def _generate_app_log_for_event(
        event: Dict[str, Any],
        app_registry: AppRegistry,
        target_dir: Path,
        chain_logs_history: Dict[str, List[Dict[str, Any]]],
        ...
        if any(field not in event for field in required_fields):
            print("Skipping event with missing fields")
            return None
        app_name = event.get("app_name", "")
        api_name = event.get("api_name", "")
        ...
        # Build event payload with additional metadata for habits with variations
        event_payload = {
            "event": {
        ...
        # Include original app_api_variations if this event was sampled from variations
        if event.get("original_app_api_variations"):
            event_payload["event"]["original_app_api_variations"] = event.get("original_app_api_variations")
        ...
        previous_logs_formatted = [
            ...
            for log in previous_chain_logs
        ]
        ...
        prompt = app_log_prompt.render(
            ...
            previous_chain_logs=json.dumps(_json_safe(previous_logs_formatted), indent=2, ensure_ascii=False),
        )

        target_dir.mkdir(parents=True, exist_ok=True)
        with open(target_dir / f"app_log_prompt_{event.get('event_id')}.txt", "w") as f:
            f.write(prompt)
        ...
        try:
            # Build response schema from API schema (using Pydantic model schemas)
            input_schema = api_schema.get("input", {})
            output_schema = api_schema.get("output", {})

            # If schema has Pydantic's $defs, use it directly; otherwise convert
            if "$defs" in input_schema or "properties" in input_schema:
                # Already a proper JSON schema from Pydantic
                input_json_schema = input_schema
            else:
                input_json_schema = _spec_to_json_schema(input_schema)

            if "$defs" in output_schema or "properties" in output_schema:
                output_json_schema = output_schema
            else:
                output_json_schema = _spec_to_json_schema(output_schema)
            ...
            # Then validate with specific API models if available
            request_payload, response_payload = _validate_with_api_models(
                app_name, api_name, raw_input, raw_output
            )
            ...
            debug_payload = {
                ...
                "llm_response": _json_safe(llm_raw),
                "response_schema": response_schema,
            }
            if payload is not None:
                debug_payload["parsed_payload"] = _json_safe(payload)
            if raw_input is not None:
                debug_payload["parsed_input"] = _json_safe(raw_input)
            if raw_output is not None:
                debug_payload["parsed_output"] = _json_safe(raw_output)
            with open(
                target_dir
                / f"app_log_raw_{event.get('event_id')}_{event.get('app_name')}_{event.get('api_name')}.json",
                "w",
            ) as f:
                json.dump(_json_safe(debug_payload), f, indent=2, ensure_ascii=False)
            print(
        ...
        with open(
            target_dir
            / f"app_log_{event.get('event_id')}_{event.get('app_name')}_{event.get('api_name')}.json",
            "w",
        ) as f:
            json.dump(_json_safe(log), f, indent=2, ensure_ascii=False)
        
        # Record log in chain history for future events in the same chain
        if chain_id:
    ...
    ## if no target app name is provided, use all.
    # target_app_name = "LLM Assistant"
    ...
    if target_app_name:
        ...
        target_app_events = [
            event
            for event in all_events
            if (event.get("app_name") or "").lower() == target_app_name_lower
        ]
    else:
    ...
    for i, event in tqdm(enumerate(target_app_events)):
        log = _generate_app_log_for_event(event, app_registry, app_specific_dir, chain_logs_history)
    ...
    if not target_app_events:
        print(f"\nNo events found for app: {target_app_name}")
    ...
    # Save results
    output_file = app_specific_dir / "debug_app_logs_merged.json"
    output_data = {
        "user_id": user_id,
        "app_name": target_app_name,
        "total_events": len(target_app_events),
        "total_app_logs": len(app_logs),
        "domains": sorted(
            {event.get("domain_name") for event in target_app_events if event.get("domain_name")}
        ),
        "app_logs": app_logs
    }

    with open(output_file, "w") as f:
        json.dump(_json_safe(output_data), f, indent=2, ensure_ascii=False)
