...
for user in "${USERS[@]}"; do
  ...
  if [[ ! -f "$app_logs_final" ]]; then
    echo "[SA-BENCH] SKIP $user_dir: missing $app_logs_final"
    continue
  fi

  echo "[SA-BENCH] Building benchmark for $user_dir"
  (
    cd "$PROJECT_ROOT"
    "$PYTHON_BIN" data_construction/build_state_abstraction_benchmark.py \
