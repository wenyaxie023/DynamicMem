# Temporal Checkpoint Evaluation Developer Manual (Legacy Archive)
...
Archive note:
- This file preserves the pre-refactor manual for traceability.
- It is not the active TCE developer contract.
- Active protocol document:
...
2. `End-to-End QA Evaluation`（全上下文，3 levels x 6 sublevels）[legacy]
...
5. Optional post-TCE final-memory QA:
   ...
   - current project policy: this stage is currently disabled by default and should be treated as explicit opt-in only
...
- slot-level LLM judge for answer atomic facts
  ...
  - `taskabc_v2` structured families must define a family-appropriate structured service object:
    ...
    - `scoring_rubric.criteria[]` must align one-to-one with the scalar leaves in `reference_output`
    - each rubric criterion should name one `path`, the exact `canonical_value` at that path, and a short semantic `description`
  ...
  - `taskabc_v2` structured-family `answer_scoring_points[]` must be positive-only field points lowered from `scoring_rubric`:
    - `point_type = "field"`
    - `polarity = "positive"`
    - each point must include:
      - `target_path`
      - `reference_value`
    - builder-side trace fields such as `source_field_path` / `output_field_path` are optional metadata, not required by eval
  - every required scalar leaf in `reference_output` must appear in at least one scoring point
...
- `point-pairability`: structured families score one required output field at a time via deterministic field points; `user_communication` uses point-based natural-language scoring against `reference_answer`
