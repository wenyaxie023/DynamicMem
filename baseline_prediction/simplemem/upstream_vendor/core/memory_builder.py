...
class MemoryBuilder:
    ...
    def clone(self, *, vector_store: VectorStore) -> "MemoryBuilder":
        other = MemoryBuilder(
        ...
        other.overlap_size = int(self.overlap_size)
        other.step_size = int(self.step_size)
        other.dialogue_buffer = [Dialogue.from_dict(item.to_dict()) for item in self.dialogue_buffer]
        other.processed_count = int(self.processed_count)
        other.previous_entries = [MemoryEntry.from_dict(item.to_dict()) for item in self.previous_entries]
        return other
...
"""
    ...
    def _parse_llm_response(
        ...
        for item in data:
            ...
            if not entry.lossless_restatement.strip():
                continue
