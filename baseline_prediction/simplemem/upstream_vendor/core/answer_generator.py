...
class AnswerGenerator:
    ...
    def generate_answer(
        ...
        *,
        ...
        # Build prompt
        prompt = self._build_answer_prompt(query, context_str, prompt_override=prompt_override)
    ...
    def _build_answer_prompt(
        ...
        *,
        ...
        """
        if prompt_override is not None:
            return f"""{prompt_override}

[Retrieved Agent Memory]
{context_str}

[Backend Constraints]
- Use only the retrieved agent memory above.
- When the task asks for evidence, prefer exact `app_log_id` values from any lineage metadata included above.
- Return only the JSON object required by the task prompt.
"""
        return f"""
