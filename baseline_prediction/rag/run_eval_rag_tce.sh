...
PREDICTION_ROOT="$PROJECT_ROOT/baseline_prediction/rag/results"
...
for user in "${USERS[@]}"; do
  ...
  if [[ -n "${PREDICTION_PATH:-}" ]]; then
    ...
    prediction_path="$(resolve_latest_named_file "$prediction_dir" "tce_results*topk${RETRIEVAL_TOP_K}*.json" || true)"
