...
class TcePipelinePackFirstAcceptance(unittest.TestCase):
    ...
    def test_pipeline_runs_task_c_v2_user_communication_with_natural_language_answer(self):
        ...
        prediction = result["predictions"][0]
        self.assertIn("It is 06:10. Nothing has been logged yet today.", seen_task_text["value"])
        self.assertIn("Write the short reminder message the assistant should send right now.", seen_task_text["value"])
        self.assertNotIn("Service family:", seen_task_text["value"])
        self.assertNotIn("Retrieval objective:", seen_task_text["value"])
    ...
    def test_pipeline_runs_task_c_only_without_state_completion_pack(self):
        benchmark = {
            "user_id": "001_user_001",
            "task_contract_version": CURRENT_TASK_CONTRACT_VERSION,
            "research_frame_version": RESEARCH_FRAME_VERSION_V2,
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            benchmark_path.write_text(
                json.dumps(_with_default_legacy_contract(benchmark), ensure_ascii=False, indent=2),
                encoding="utf-8",
            )
            app_logs_path.write_text(json.dumps(app_logs, ensure_ascii=False, indent=2), encoding="utf-8")
            result = run_pipeline(
        ...
        prediction = result["predictions"][0]
        self.assertEqual(prediction["snapshot_state"], {})
        self.assertEqual(prediction["evidence"], {})
        self.assertEqual(prediction["metadata"]["task_selection"], "task_c_only")
        self.assertEqual(
            prediction["metadata"]["task_a"],
            {"enabled": False, "skipped_by_task_selection": True},
        )
        item = prediction["rq3_apply_answers"]["habits_state:morning_walk"]["items"][0]
        self.assertEqual(item["answer"], "Send a reminder that the walk starts at 06:30.")
